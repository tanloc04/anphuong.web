...
const UserManagement = () => {
    ...
    const { update, toggleStatus, isPending } = useUserMutations(toast);

    const openNew = () => { setSelectedUser(null); setModalVisible(true); };
    const openEdit = (user: any) => { setSelectedUser(user); setModalVisible(true); };

    const handleSave = (formData: any) => {
        const options = { onSuccess: () => { setModalVisible(false); refetch(); } };
    ...
    const userBodyTemplate = (rowData: any) => (
        <div className="flex align-items-center gap-2">
            <Avatar label={rowData.username?.charAt(0).toUpperCase()} shape="circle" className="bg-blue-100 text-blue-700" />
    ...
    const roleBodyTemplate = () => <Tag value="Customer" severity="info" rounded />;
    ...
    return (
        <ManagementLayout
            ...
            searchTerm={keyword}
            onSearchChange={(val: any) => { setKeyword(val); setLazyParams(prev => ({ ...prev, first: 0, page: 0 })); }}
            onCreate={openNew}
        ...
        >
            ...
            <DataTable 
                value={users} 
                lazy paginator first={lazyParams.first} rows={lazyParams.rows} totalRecords={totalRecords}
                onPage={onPage} loading={isLoading || isFetching}
                tableStyle={{ minWidth: '60rem' }} emptyMessage="Không tìm thấy người dùng nào."
                className="p-datatable-sm" stripedRows size="small"
            >
                <Column field="id" header="ID" style={{ width: '50px' }} className="text-center text-xs text-gray-500"/>
            ...
            <UserForm 
                visible={modalVisible} onHide={() => setModalVisible(false)} 
                onSave={handleSave} initialData={selectedUser} loading={isPending}
            />
        </ManagementLayout>
    );
};
