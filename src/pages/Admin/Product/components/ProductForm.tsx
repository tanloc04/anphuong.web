...
const ProductForm = ({
  ...
  onClose,
  loading,
}: ProductFormProps & { loading?: boolean }) => {
  ...
  useEffect(() => {
    ...
    // KHI CÓ DATA UPDATE TỪ API TRẢ VỀ
    if (productDetail) {
      setFormData({
        id: productDetail.id,
        ...
        isCustomize: productDetail.isCustomize ?? false,
        categoryId: productDetail.categoryId,

        // 👇 MÓC VÀO OBJECT detailImage TỪ BACKEND TRẢ VỀ
        image1: productDetail.detailImage?.image1 || null,
        image2: productDetail.detailImage?.image2 || null,
        image3: productDetail.detailImage?.image3 || null,
        image4: productDetail.detailImage?.image4 || null,
      });

      // 👇 GÁN LINK CLOUDINARY VÀO PREVIEW ĐỂ NÓ HIỆN LÊN GIAO DIỆN
      const urls = {
        image1: productDetail.detailImage?.image1 || null,
        image2: productDetail.detailImage?.image2 || null,
        image3: productDetail.detailImage?.image3 || null,
  ...
  const renderImageUpload = (key: keyof PreviewUrls, label: string) => (
    <div className="relative group h-full">
      <label htmlFor={key} className="cursor-pointer block h-full">
        <div
          className={`border-2 border-dashed ${previewUrls[key] ? "border-indigo-500 bg-indigo-50" : "border-gray-300 bg-gray-50/50"} rounded-lg p-2 h-full min-h-[120px] flex flex-col items-center justify-center hover:border-indigo-500 hover:bg-indigo-50 transition-colors`}
        >
          {previewUrls[key] ? (
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full relative">
      <div className="flex flex-col lg:flex-row gap-8 pb-24">
        <div className="lg:w-[70%] space-y-6">
          <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
            <h3 className="font-bold text-gray-800 mb-4 border-b pb-2 flex items-center gap-2">
              <i className="pi pi-info-circle text-blue-500"></i> Thông tin cơ
              bản
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <InputNumber
                  ...
                  onValueChange={(e) => handleValueChange("price", e.value)}
                  mode="currency"
                  currency="VND"
                  locale="vi-VN"
                  className="w-full"
              ...
              <div>
                ...
                <InputNumber
                  ...
                  onValueChange={(e) => handleValueChange("discount", e.value)}
                  mode="currency"
                  currency="VND"
                  locale="vi-VN"
                  className="w-full"
          ...
          <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
            <h3 className="font-bold text-gray-800 mb-4 border-b pb-2 flex items-center gap-2">
              <i className="pi pi-arrows-alt text-orange-500"></i> Kích thước
              (Mét)
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              <div>
                <label htmlFor="longSize" className={labelClass}>
                  Dài
                </label>
                <InputNumber
                  id="longSize"
                  ...
                  onValueChange={(e) => handleValueChange("longSize", e.value)}
                  minFractionDigits={0}
                  maxFractionDigits={2}
                  className="w-full"
                  suffix=" m"
              ...
              <div>
                <label htmlFor="widthSize" className={labelClass}>
                  Rộng
                </label>
                <InputNumber
                  id="widthSize"
                  ...
                  onValueChange={(e) => handleValueChange("widthSize", e.value)}
                  minFractionDigits={0}
                  maxFractionDigits={2}
                  className="w-full"
                  suffix=" m"
              ...
              <div>
                <label htmlFor="heightSize" className={labelClass}>
                  Cao
                </label>
                <InputNumber
                  id="heightSize"
                  ...
                  onValueChange={(e) =>
                    handleValueChange("heightSize", e.value)
                  }
                  minFractionDigits={0}
                  maxFractionDigits={2}
                  className="w-full"
              ...
              <div className="sm:col-span-3 flex align-items-center mt-3 p-3 bg-blue-50 rounded-lg border border-blue-100">
                <InputSwitch
                  id="isCustomize"
                  checked={formData.isCustomize}
                  onChange={(e) => handleValueChange("isCustomize", e.value)}
                />
                <div className="ml-3">
                  <label
                    htmlFor="isCustomize"
                  ...
                  <small className="text-gray-500 block">
                    Khách hàng có thể tự nhập kích thước Dài/Rộng/Cao khi đặt
                    hàng sản phẩm này.
                  </small>
                </div>
              </div>
        ...
        {/* 🎨 ĐÃ LÀM LẠI KHU VỰC ẢNH BÊN PHẢI */}
          ...
          <div className="sticky top-4 space-y-6">
            <div className="bg-white p-5 rounded-xl border shadow-sm">
              ...
              <div className="space-y-3">
                {/* Ảnh chính to bự ở trên */}
                <div className="h-56">
                  {renderImageUpload("image1", "Ảnh chính (Đại diện)")}
                </div>

                {/* 3 Ảnh phụ xếp ngang ở dưới */}
                <div className="grid grid-cols-3 gap-3 h-28">
                  {renderImageUpload("image2", "Ảnh phụ 1")}
                  {renderImageUpload("image3", "Ảnh phụ 2")}
                  {renderImageUpload("image4", "Ảnh phụ 3")}
                </div>
              </div>

              <small className="text-gray-500 block mt-4 text-center italic">
                * Ảnh chính sẽ tự động làm ảnh đại diện cho sản phẩm.
              </small>
            </div>
          </div>
        </div>
      ...
      <div className="flex justify-end gap-3 mt-4 pt-4 border-t sticky bottom-0 bg-white/95 backdrop-blur-sm z-50 py-4 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] -mx-6 px-6 rounded-b-lg">
        ...
        />
        <Button
          label={isUpdateMode ? "Lưu Thay Đổi" : "Tạo Sản Phẩm"}
          icon="pi pi-check"
          severity="success"
          type="submit"
          loading={loading}
